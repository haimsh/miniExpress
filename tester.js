...
//logger.activate();
logger.startCallBack('tester.js:main');
...
function closeApp(app, duration) {
    setTimeout(function () {
        logger.startCallBack('tester.js: Closing app server');
        app.close();
        logger.endCallBack('tester.js: Closing app server');
    }, duration);
}
...
function testSite() {
    ...
    fs.readdir(testDir, function (err, files) {
        logger.startCallBack('tester.js:wwwFilesIterator');
        if (err) {
            throw err;
        }
        for (file in files) {
            ...
            (function (fileName) {
                ...
                http.request(requestOption, function (respond) {
                    logger.startCallBack('tester.js:respondFromServer for ' +
                        fileName);
                    var content = [];
                    respond.on('data', function (data) {
                        logger.startCallBack('tester.js:dataOnRespond for ' +
                            fileName);
                        content.push(data);
                        logger.endCallBack('tester.js:dataOnRespond for ' +
                            fileName);
                    });
                    respond.on('end', function () {
                        logger.startCallBack('tester.js:endRespond for ' +
                            fileName);
                        ...
                        fs.readFile(path.normalize(testDir + fileName),
                            ...
                            logger.startCallBack('tester.js:compareContent ' +
                                'for' + fileName);
                            ...
                            logger.endCallBack('tester.js:compareContent for' +
                                fileName);
                        });
                    });
                    logger.endCallBack('tester.js:respondFromServer for ' +
                        fileName);
                }).end();
            }('/' + files[file]));
        }
        logger.endCallBack('tester.js:wwwFilesIterator');
    });
}
...
function multiServer() {
    ...
    function testSourceSite(site){
        ...
        http.request(requestOption, function (res) {
            logger.startCallBack('tester.js: multiServer respond for site ' +
                site);
            ...
            logger.endCallBack('tester.js: multiServer respond for site ' +
                site);
...
function testKeepAlive(httpVer, connection, wantedResult) {
    ...
    socket = net.connect({
        ...
        logger.startCallBack('tester.js: keep - connect');
        socket.write('GET /smallFile.txt HTTP/'+httpVer + '\r\n' +
            (connection !== 'none' ? 'Connection: ' + connection + '\r\n' : '')
            + '\r\n', function () {});
        logger.endCallBack('tester.js: keep - connect');
    });
    reader = readline.createInterface(socket, process.stdout, false, false);
    reader.on('line', function (line) {
        logger.startCallBack('tester.js: keep - line in respond');
        ...
        logger.endCallBack('tester.js: keep - line in respond');
    });
    reader.on('end', function () {
        logger.startCallBack('tester.js: keep - respond end');
        if (!connectionHeader) {
            console.error('testKeep: No Connection header returned');
        }
        logger.endCallBack('tester.js: keep - respond end');
    });
